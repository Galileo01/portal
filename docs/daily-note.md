...
- [x] 资源发布
  ...
  - [x] 发布成功后 ，输出该页面的访问地址或者提示用户是否立即打开
...
- [ ] 排序、搜索?
- [x] 多窗口情况下 用户登出、登录，
  - [x] 监听 localstorage 中 token 的更新 ，刷新用户数据
  - [x] 登出 删除 本地数据？
- [ ] 排序最近靠前
...
- [x] feat-config 存储 和上传 时 ，propsSchema 以及一些其他字段 不进行存储

- [ ] ~~dom-to-img 跨域问题~~

- [ ] 已有页面和模板的相互转化

- [x] 模板 type 字段的填充！！！！！！---client 端填充
...
- [x] 更新后 title 问题
  ...
  - 发布表单根据请求数据填入 、 去掉从 url 载入 title 的逻辑
...
- [x] editer 下 不生效 元信息配置

#### 样式配置

- [x] 分类-背景 移除“渐变” 相关文字即功能 or 添加 渐变相关!!!!
- [ ]
...
准备一个比较好的资源创建流程，样式风格好，可以只管展示的用例
